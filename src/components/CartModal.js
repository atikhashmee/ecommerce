...
import {
  ...
  export default function CartModal ({modalVisible, setmodalVisible}) {
    let [variationImages, setvariationImages] = useState([
      ...
      />
      <Container style={{ width: wp('100%'), height: hp('70%'), position: 'absolute', bottom: 0, left: 0 }}>
            <Grid style={{ padding: 10 }}>
                <Row style={{ borderBottomWidth: 1, borderBottomColor: '#eee', paddingBottom: 10 }}>
                    <Col style={{ flexDirection:"row" }}>
                        <View style={styles.imageContainer}>
                            <Image source={{ uri: variationImages[0] }}  style={AppStyle.image} />
                        </View>
                ...
                <Row>
                    <Col style={{ paddingVertical: 8, borderBottomColor: '#eee', borderBottomWidth: 1 }}> 
                        <View>
                            <Text style={{ fontSize: 20, paddingLeft: 10 }}>Color Family</Text>
                        </View>
                        <View style={{ flexDirection: 'row' }}>
                            {variationImages.map((img, ind) => 
                            <View key={ind} style={{ width: 50, marginHorizontal: 10, height: 50 }}>
