            ...
            <ScrollView showsVerticalScrollIndicator={false} style={ {backgroundColor: '#fff'}}>
                <View style={{ flex: 1 }}>
                    { categories.map((item, index) => {
                        return <Pressable onPress={() => categoryToggle(item.id)} key={index + item.id} 
                        style={{...AppStyle.categoryEachBox, marginRight: 0, backgroundColor: item.id === currentCatId?'#fff':'#eee', borderBottomColor: '#d4d4d4', borderBottomWidth: 1, padding: 10}}>
                            <View style=
                            {{...AppStyle.categoryIconHolder, backgroundColor: item.id === currentCatId?'#fff':'#eee', padding: 0}}>
